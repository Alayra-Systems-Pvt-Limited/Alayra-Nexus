...
test('claiming creates the owner and shows the recovery key exactly once', async () => {
  ...
  // Step 3 — workspace name is optional; leave it blank and continue.
  await expect(page.getByText('Name your workspace')).toBeVisible();
  await page.getByRole('button', { name: 'Continue' }).click();

  // Step 4 (C6) — the backup passphrase. Generated by default, so read what it offered: the kit
  // screen will demand it back, and a test that typed its own would not be exercising that.
  ...
  // The account recovery key is on screen, in the hyphenated shape a person can write down. The
  // backup passphrase sits beside it and is words rather than hex, so this still names one of them.
  await expect(page.getByText(/^[0-9a-f]{4}(-[0-9a-f]{4}){7}$/)).toBeVisible();

  // The kit gate: no way past it until the passphrase is pasted back. A real browser is the only
  // place this is proven — it is a disabled button and a string comparison, and both are the sort
  // of thing that works in a unit test and not on a page.
