 ...
 *
 * Small, and worth its own function because the first version wrote "a OpenRouter pool" and "This
 * looks like OpenRouter key" into an error an operator reads while something is already going
 * wrong. On the vowel test rather than a list: every name here starts with a letter, and the rule
 * then keeps working for a provider nobody has added yet.
 */
...
export function keyProviderMismatch(providerSlug: string, apiKey: string): string | null {
  ...
  return `This looks like ${withArticle(providerLabel(issuer))} key, but this pool is `
    + `${providerLabel(providerSlug)}. A pool serves one provider — add `
    + `${withArticle(providerLabel(issuer))} pool for this key, or paste `
    + `${withArticle(providerLabel(providerSlug))} key here.`;
}
